...
float sin_approx(float x)
{
    ...
    while (x >  M_PI_FLOAT) x -= (2.0f * M_PI_FLOAT);                                 // always wrap input angle to -PI..PI
    while (x < -M_PI_FLOAT) x += (2.0f * M_PI_FLOAT);
    if (x >  (0.5f * M_PI_FLOAT)) x =  (0.5f * M_PI_FLOAT) - (x - (0.5f * M_PI_FLOAT));   // We just pick -90..+90 Degree
    else if (x < -(0.5f * M_PI_FLOAT)) x = -(0.5f * M_PI_FLOAT) - ((0.5f * M_PI_FLOAT) + x);
    float x2 = x * x;
...
{
    return sin_approx(x + (0.5f * M_PI_FLOAT));
}
...
float atan2_approx(float y, float x)
{
    ...
    if (absY > absX) res = (M_PI_FLOAT / 2.0f) - res;
    if (x < 0) res = M_PI_FLOAT - res;
    if (y < 0) res = -res;
    return res;
}
